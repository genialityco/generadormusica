...
export default function Home() {
  const [prompt, setPrompt] = useState(
    "Colombia 4.0 a generar innovación, con inteligencia artificial e inteligencia colombiana"
  );
      ...
      <main className="flex flex-col gap-8 row-start-2 items-center sm:items-start">
        ...
        {data && (
          <div>
            <p>{data?.audio_url}</p>
            {data?.audio_url && (
              <audio src={data?.audio_url} controls autoPlay />
            )}
            <h1>Data Fetched</h1>
        ...
        <button
          style={{ height: "50px", width: "200px", border: "1px solid blue" }}
          onClick={generar}
        >
          {" "}
          Generar{" "}
